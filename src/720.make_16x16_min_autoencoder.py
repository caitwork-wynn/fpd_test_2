...
# -*- coding: utf-8 -*-
"""
720.make_16x16_min_autoencoder.py
16x16 이미지 최소 크기 Autoencoder 학습 스크립트

- data/base/not-labeled의 이미지를 사용
- 112x112로 랜덤 크롭 + 좌우 반전 증강
- 7x7 그리드로 분할하여 16x16 패치 생성 (리사이즈 없음)
- 메모리 기반 학습으로 속도 최적화
- 최소 파라미터 모델 (채널: 16→32→64, latent_dim: 32)
"""
...
from model_defs.autoencoder_16x16_min import MinimalAutoencoder16x16
...
def visualize_reconstruction(model, dataset, device, save_path, epoch, config):
    ...
    # 제목 추가
    fig.suptitle(f'Epoch {epoch} - Top: Original, Bottom: Reconstructed (16x16 Minimal)', fontsize=16)
...
def main():
    """메인 함수"""
    print("=" * 60)
    print("16x16 Minimal Autoencoder Training")
    print("=" * 60)

    # 설정 로드
    config_path = Path(__file__).parent / 'ae_16x16_min_config.yml'
    ...
    for epoch in range(start_epoch + 1, config['training']['epochs'] + 1):
        ...
        if test_loss < best_loss - min_delta:
            ...
            # 현재 모델을 ae_16x16_min_current.pth로도 저장
