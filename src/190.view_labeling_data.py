"""
라벨링 데이터 뷰어
- data/base 내의 training_data 폴더를 선택하여 labels.txt를 읽고 학습 이미지에 xy좌표를 표시
...
class LabelingDataViewer:
    def __init__(self, root):
        self.root = root
        self.root.title("라벨링 데이터 뷰어")
        self.root.geometry("1400x900")

        # 데이터 변수
        self.base_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "base")
    ...
    def create_widgets(self):
        ...
        # 폴더 선택 콤보박스
        tk.Label(control_frame, text="폴더 선택:").pack(side=tk.LEFT, padx=5)
        self.folder_combo = ttk.Combobox(control_frame, width=40, state="readonly")
    ...
    def load_folders(self):
        """data/base 폴더 내의 training_data 폴더들을 로드"""
        try:
            folders = []
            for item in os.listdir(self.base_path):
                item_path = os.path.join(self.base_path, item)
                if os.path.isdir(item_path):
                    # labels.txt 파일이 있는 폴더만 추가
                    labels_file = os.path.join(item_path, "labels.txt")
                    if os.path.exists(labels_file):
                        folders.append(item)

            folders.sort(reverse=True)  # 최신 폴더가 위로
            ...
            messagebox.showerror("오류", f"폴더 로드 중 오류 발생:\n{str(e)}")

    def on_folder_selected(self, event):
        """폴더가 선택되었을 때"""
        selected = self.folder_combo.get()
        if selected:
            self.current_folder = os.path.join(self.base_path, selected)
            self.load_labels()
            self.current_page = 0
            self.display_page()
