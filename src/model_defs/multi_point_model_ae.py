...
class FPDAttentionModel(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        self.use_autoencoder = config.get('use_autoencoder', False)

        # 입력 특징 차원 (autoencoder 사용 시 16차원, 아니면 기존 903차원)
        if self.use_autoencoder:
            self.input_dim = config.get('encoder_latent_dim', 16)
        else:
            self.input_dim = config.get('input_dim', 903)

        # Autoencoder 사용 시 다른 아키텍처
        if self.use_autoencoder:
            # 16차원 latent vector를 직접 사용
            # 패치 구조 없이 직접 MLP 구조 사용
            fusion_dim = 512
        ...
        # 모듈 생성 (Autoencoder 사용 여부에 따라 다름)
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.use_autoencoder:
            # Autoencoder 특성을 직접 처리
            fused = self.feature_projection(features)  # [B, 16] -> [B, 512]
        else:
            # 기존 패치 기반 처리
            # 특징 재구성
        ...
        return {
            ...
            'coordinates': norm_coordinates,  # [B, 8] 정규화된 좌표
            'pixel_coordinates': coordinates  # [B, 8] 픽셀 좌표
        }
    ...
    def compute_loss(self, outputs: Dict[str, torch.Tensor],
        ...
        Args:
            ...
            targets: [batch_size, 8] 실제 좌표값 (정규화된 0~1)
...
class DataSetWithAutoencoder(Dataset):
    """Autoencoder를 사용한 다중 포인트 검출 데이터셋"""
    ...
    def __init__(self,
                 ...
                 extract_features: bool = True,  # Autoencoder는 항상 사전 추출
                 encoder_path: str = None):
        """
        Args:
            encoder_path: 사전 학습된 autoencoder 경로
        """
        ...
        # 특성과 타겟 저장
        self.features = []  # encoder 특성 저장
        self.targets = []
        self.metadata = []

        # 특성 사전 추출
        self.precompute_encoder_features()

        print(f"{mode.upper()} 데이터셋 (Autoencoder): {len(self.features)}개 샘플")
    ...
    def load_encoder(self, encoder_path: str):
        ...
        # Encoder 생성
        encoder = ResidualEncoder16x16(input_channels=1, latent_dim=16)

        # 체크포인트 로드
        if encoder_path and os.path.exists(encoder_path):
            print(f"Encoder 로드: {encoder_path}")
            checkpoint = torch.load(encoder_path, map_location=device)

            # encoder state dict 찾기
            if 'encoder_state_dict' in checkpoint:
                encoder.load_state_dict(checkpoint['encoder_state_dict'])
            elif 'model_state_dict' in checkpoint:
                # 전체 autoencoder에서 encoder 부분만 추출
                encoder_dict = {k.replace('encoder.', ''): v
                               for k, v in checkpoint['model_state_dict'].items()
                               if k.startswith('encoder.')}
                encoder.load_state_dict(encoder_dict)
            else:
                raise ValueError(f"Cannot find encoder weights in {encoder_path}")
        else:
            print(f"Warning: Encoder path not found: {encoder_path}")
            print("Using randomly initialized encoder")

        encoder.to(device)
        encoder.requires_grad_(False)  # 가중치 동결
        return encoder

    def prepare_image_for_encoder(self, image: np.ndarray) -> torch.Tensor:
        """이미지를 encoder 입력 형식으로 변환"""
        # BGR to Grayscale
        if len(image.shape) == 3:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        else:
        ...
        # Tensor 변환 및 정규화 [0, 255] -> [0, 1]
        tensor = torch.FloatTensor(gray_16).unsqueeze(0).unsqueeze(0) / 255.0

        return tensor

    def precompute_encoder_features(self):
        """모든 데이터의 encoder 특성을 미리 추출"""
        print(f"{self.mode.upper()} 데이터셋 Encoder 특성 추출 중...")

        device = next(self.encoder.parameters()).device

        for sample in tqdm(self.data, desc="Encoder 특성 추출", disable=False):
            # 이미지 로드
            ...
            # Encoder 입력 준비
            image_tensor = self.prepare_image_for_encoder(image_112).to(device)

            # Encoder로 특성 추출
            with torch.no_grad():
                latent, _ = self.encoder(image_tensor)  # residuals는 사용 안 함
                latent = latent.cpu()  # [1, 16]
            ...
            # 저장
            self.features.append(latent.squeeze(0))  # [16]
            ...
            if self.mode == 'train' and self.augment:
                for aug_idx in range(self.augment_count):
                    ...
                    # Encoder 입력 준비
                    cropped_tensor = self.prepare_image_for_encoder(cropped_112).to(device)

                    # Encoder로 특성 추출
                    with torch.no_grad():
                        latent_aug, _ = self.encoder(cropped_tensor)
                        latent_aug = latent_aug.cpu()
                    ...
                    # 증강 데이터 저장
                    self.features.append(latent_aug.squeeze(0))
...
class DataSet(Dataset):
    """다중 포인트 검출 데이터셋 (PyTorch 버전) - 기존 버전 유지"""
...
class PointDetector:
    ...
    def __init__(self, config: Dict, device):
        ...
        # Autoencoder 사용 여부 확인
        self.use_autoencoder = config.get('features', {}).get('use_autoencoder', False)

        if self.use_autoencoder:
            # Autoencoder 사용 시 특성 차원은 16
            actual_input_dim = config['features'].get('encoder_latent_dim', 16)
            print(f"Using Autoencoder features with dimension: {actual_input_dim}")

            # 특징 추출용 detector는 필요 없음 (encoder가 대체)
            self.detector = None
        else:
            # 기존 특징 추출 방식
        ...
        if self.use_fpd_architecture:
            if self.use_autoencoder:
                print("Using FPD Architecture with Autoencoder features")
            else:
            ...
            fpd_config = {
                ...
                'encoder_latent_dim': actual_input_dim if self.use_autoencoder else None
            }

            # FPD 모델 생성
            model = FPDAttentionModel(fpd_config)
            model.to(device)
            ...
            pytorch_config = {
                'architecture': {
                    ...
                    'hidden_dims': [384, 256] if not self.use_autoencoder else [128, 64],  # Autoencoder는 작은 네트워크
                    'output_dim': 8,
            ...
            # 올바른 차원으로 모델 재생성
            model = ConfigurableMLPModel(pytorch_config['architecture'])
            model.to(device)
...
PointDetectorDataSetWithAutoencoder = DataSetWithAutoencoder
